 ...
 * [SwipeableState2] with the default animation clock, use [rememberSwipeableState].
 *
...
open class SwipeableState2<T>(
    ...
    companion object {
        /**
         * The default [Saver] implementation for [SwipeableState2].
         */
        fun <T : Any> Saver(
            animationSpec: AnimationSpec<Float>,
            confirmStateChange: (T) -> Boolean
        ) = Saver<SwipeableState2<T>, T>(
            save = { it.currentValue },
            restore = { SwipeableState2(it, animationSpec, confirmStateChange) }
        )
    }
}
 ...
 *
 * To access this information, use [SwipeableState2.progress].
 *
...
/**
 * Create and [remember] a [SwipeableState2] with the default animation clock.
 *
 * @param initialValue The initial value of the state.
 * @param animationSpec The default animation that will be used to animate to a new state.
 * @param confirmStateChange Optional callback invoked to confirm or veto a pending state change.
...
/**
 * Create and [remember] a [SwipeableState2] which is kept in sync with another state, i.e.:
 *  1. Whenever the [value] changes, the [SwipeableState2] will be animated to that new value.
 *  2. Whenever the value of the [SwipeableState2] changes (e.g. after a swipe), the owner of the
 *  [value] will be notified to update their state to the new value of the [SwipeableState2] by
 *  invoking [onValueChange]. If the owner does not update their state to the provided value for
 *  some reason, then the [SwipeableState2] will perform a rollback to the previous, correct value.
 */
@Composable
@ExperimentalMaterialApi
internal fun <T : Any> rememberSwipeableStateFor(
    ...
    LaunchedEffect(value, forceAnimationCheck.value) {
        if (value != swipeableState2.currentValue) {
            swipeableState2.animateTo(value)
        }
    }
    DisposableEffect(swipeableState2.currentValue) {
        if (value != swipeableState2.currentValue) {
            onValueChange(swipeableState2.currentValue)
 ...
 *
 * When a swipe is detected, the offset of the [SwipeableState2] will be updated with the swipe
 * delta. You should use this offset to move your content accordingly (see `Modifier.offsetPx`).
 * When the swipe ends, the offset will be animated to one of the anchors and when that anchor is
 * reached, the value of the [SwipeableState2] will also be updated to the state corresponding to
...
/**
 * Contains useful defaults for [swipeable] and [SwipeableState2].
 */
object SwipeableDefaults {
    /**
     * The default animation used by [SwipeableState2].
     */
...
@ExperimentalMaterialApi
internal val <T> SwipeableState2<T>.PreUpPostDownNestedScrollConnection: NestedScrollConnection
