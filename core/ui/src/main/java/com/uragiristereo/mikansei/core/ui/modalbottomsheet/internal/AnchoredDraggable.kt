 ...
 * @param confirmValueChange Optional callback invoked to confirm or veto a pending animation change.
 */
@Stable
internal class AnchoredDraggableState<T>(
    ...
    internal val confirmAnimate: suspend (progress: Float) -> Boolean = { true },
) {
    /**
    ...
    constructor(
        ...
        confirmAnimate: suspend (progress: Float) -> Boolean = { true },
    ) : this(
        initialValue,
        positionalThreshold,
        velocityThreshold,
    ...
    val progress: Float by derivedStateOf(structuralEqualityPolicy()) {
        try {
            ...
            when {
                maxOffset == minOffset -> 1f
                else -> (1f - (offset - minOffset) / (maxOffset - minOffset)).coerceIn(0f, 1f)
            }
        } catch (_: Exception) {
            0f
        }
